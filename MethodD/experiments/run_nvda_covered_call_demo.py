...
def _ensure_snapshot_dir() -> str:
    base_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'snapshots')
...
REQUIRED_CHAIN_COLUMNS = ['contractSymbol', 'expiry', 'strike', 'bid', 'ask', 'last', 'iv', 'optionType']
...
def _match_contract(chain_df: pd.DataFrame, key: Dict[str, object], relax_strike: bool) -> Dict[str, object]:
    """严格匹配 contractSymbol + expiry + strike + type"""
    target = chain_df[(chain_df['optionType'] == key['optionType']) &
    ...
    if target.empty:
        ...
        candidates = chain_df[(chain_df['optionType'] == key['optionType']) &
                              (chain_df['expiry'] == key['expiry'])].copy()
        if candidates.empty:
...
def _validate_manifest(manifest: Dict[str, object], require_t5: bool) -> None:
    ...
    missing_contract = [field for field in REQUIRED_CONTRACT_FIELDS if not contract_key.get(field)]
    if missing_contract:
...
def _validate_snapshot(snapshot: Dict[str, object], label: str) -> None:
    ...
    missing_cols = [col for col in REQUIRED_CHAIN_COLUMNS if col not in chain_df.columns]
    if missing_cols:
        raise ValueError(f"{label} 快照字段缺失: {missing_cols}")


def _load_snapshot_from_manifest(manifest: Dict[str, object], key: str) -> Dict[str, object]:
...
def run_strict_capture(ticker: str) -> None:
    ...
    print(f"提示：t5 目标交易日为 {target_date}，请在该日期或之后执行 --strict-t5 生成 t5 并复算。")
...
def run_t5_strict(manifest: Dict[str, object], relax_strike: bool) -> Dict[str, object]:
    ...
    return _run_analysis(t0_snapshot, t5_snapshot, manifest, relax_strike, data_mode='STRICT_CAPTURE')


def _run_analysis(t0_snapshot: Dict[str, object], t5_snapshot: Dict[str, object],
                  manifest: Dict[str, object], relax_strike: bool, data_mode: str) -> Dict[str, object]:
    ...
    try:
        ...
        if t5_row['substituted']:
            ...
            row.update({
                ...
                'substitute_contract_symbol': substitute_info.get('substitute_contract_symbol')
            })
...
def run_demo_offline(relax_strike: bool) -> None:
    ...
    _run_analysis(t0_snapshot, t5_snapshot, manifest, relax_strike=relax_strike,
...
def main() -> None:
    ...
    parser.add_argument('--relax-strike', action='store_true', help='允许 t5 替代 strike')
    parser.add_argument('--strict-t0', action='store_true', help='严格模式：抓取 t0 快照并落盘')
    parser.add_argument('--strict-t5', action='store_true', help='严格模式：抓取 t5 快照并复算')
