...
class WalkForwardValidator:
    ...
    @staticmethod
    def split_walk_forward(data: pd.DataFrame,
                          train_size: int = 60,
                          test_size: int = 20,
                          step_size: int = 10) -> List[Tuple[pd.DataFrame, pd.DataFrame]]:
        """
        生成 Walk-Forward 分割
    ...
    @staticmethod
    def evaluate_walk_forward(splits: List[Tuple[pd.DataFrame, pd.DataFrame]],
                             strategy_func) -> Dict:
        """
        评估 Walk-Forward 结果
        ...
        for i, (train_data, test_data) in enumerate(splits):
            ...
            oos_performance = {
                ...
                'degradation': train_result.get('total_return', 0) - test_result.get('total_return', 0),
    ...
    def run_ablation_study(data: pd.DataFrame,
        ...
        for ablation_name, ablation_func in ablation_configs.items():
            ...
            results['ablation_results'][ablation_name] = {
                ...
                'impact': {
                    'return_change': ablation_result.get('total_return', 0) - base_result.get('total_return', 0),
                    'sharpe_change': ablation_result.get('sharpe_ratio', 0) - base_result.get('sharpe_ratio', 0),
                    'drawdown_change': ablation_result.get('max_drawdown', 0) - base_result.get('max_drawdown', 0)
                }
            }
...
class ComparisonExperiment:
    ...
    @staticmethod
    def run_comparison(data: pd.DataFrame,
