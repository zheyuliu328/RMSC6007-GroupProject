...
def main():
    ...
    for i, window in enumerate(earnings_info["windows"], 1):
        ...
        print(
            f"     窗口: {window['window_start'].strftime('%Y-%m-%d')} ~ {window['window_end'].strftime('%Y-%m-%d')}"
        )
    ...
    print(
        f"   - 做空成本: {'低（< 2%）' if constraints['eligible_for_short'] else '高（>= 2%）'}"
    )
    print()
    ...
    print(
        f"   - 避开财报窗口: {[w['earnings_date'].strftime('%Y-%m-%d') for w in earnings_info['windows']]}"
    )
    print()
