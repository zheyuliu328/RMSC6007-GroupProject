...
def _extract_chain_row(
    chain_df: pd.DataFrame, contract_symbol: str
) -> Dict[str, object]:
...
def _resolve_snapshot_files(
    snapshot_info: object, run_id: str = None
) -> Dict[object, str]:
...
def _filter_chain_by_expiry(
    chain_df: pd.DataFrame, expiry_value: object
) -> pd.DataFrame:
    if expiry_value is None:
        return chain_df
...
def build_sample_table() -> pd.DataFrame:
    ...
    for manifest in manifests:
        ...
        if isinstance(manifest.get("expiries"), list):
            expiry_keys = [
                _normalize_expiry_key(item) for item in manifest.get("expiries")
            ]
        else:
            expiry_key = _normalize_expiry_key(
                contract_key.get("expiry") or contract_key.get("expiry_epoch")
            )
        ...
        if not expiry_keys:
            expiry_keys = list(
                {
                    key
                    for key in list(t0_map.keys()) + list(t5_map.keys())
                    if key is not None
                }
            )
        if not expiry_keys:
        ...
        for expiry_key in expiry_keys:
            ...
            merged = pd.merge(
                t0_chain, t5_chain, on="contractSymbol", suffixes=("_t0", "_t5")
            )
            if merged.empty:
                continue
            ...
            for _, row in merged.iterrows():
                if (
                    "expiry_t0" in row
                    and "expiry_t5" in row
                    and row["expiry_t0"] != row["expiry_t5"]
                ):
                    continue
                if (
                    "strike_t0" in row
                    and "strike_t5" in row
                    and pd.notna(row["strike_t5"])
                ):
                    if float(row["strike_t0"]) != float(row["strike_t5"]):
                        continue

                t0_mid = _calc_mid(
                    row.get("bid_t0"), row.get("ask_t0"), row.get("last_t0")
                )
                t5_mid = _calc_mid(
                    row.get("bid_t5"), row.get("ask_t5"), row.get("last_t5")
                )
                iv_t0 = row.get("iv_t0")
                iv_t5 = row.get("iv_t5")
                ...
                sample_row = {
                    ...
                    "iv_t5": iv_t5,
                    "iv_change": None
                    if iv_t0 is None or iv_t5 is None
                    else float(iv_t5) - float(iv_t0),
                    "bid_t0": row.get("bid_t0"),
                    "ask_t0": row.get("ask_t0"),
    ...
    sample_df["factor_a"] = IVFactorDefinition.compute_factor_version_a(
        sample_df["iv_t0"]
    )
    sample_df["factor_b"] = IVFactorDefinition.compute_factor_version_b(
        sample_df["iv_t0"]
    )
...
def _print_gate_stats(run_df: pd.DataFrame, use_quantile: bool) -> None:
    ...
    print(
        f"- MACD_gate:{macd_trigger.sum()} / {len(run_df)} ({_ratio(macd_trigger):.2%})"
    )
    print(f"- BB_gate:  {bb_trigger.sum()} / {len(run_df)} ({_ratio(bb_trigger):.2%})")
    print(
        f"- 交集:     {triple_trigger.sum()} / {len(run_df)} ({_ratio(triple_trigger):.2%})"
    )
...
def _attach_triple_gate_signals(sample_df: pd.DataFrame) -> pd.DataFrame:
    ...
    run_df = (
        sample_df[run_cols].drop_duplicates(subset=["run_id"]).reset_index(drop=True)
    )
...
def _build_bucket_stats(
    sample_df: pd.DataFrame, label: str, bucket_col: str
    ...
    for bucket_id, subset in valid.groupby("_bucket"):
        ic_stats = FactorMetrics.spearman_ic_tstat(
            subset["factor_b"], subset["iv_change"]
        )
        rows.append(
            {
                "factor": "factor_b",
...
def build_stats_table(sample_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for factor_name, factor_series in factors.items():
        for target_name, target_series in targets.items():
            ...
            reg_stats = FactorMetrics.linear_regression_stats(
                factor_series, target_series
            )
            partial_ic = FactorMetrics.partial_spearman_ic(
                factor_series, target_series, control_vars
            )
    ...
    if "spread_t0" in sample_df.columns:
        ...
        if pd.notna(median_spread):
            for label, subset in [
                ("spread_low", sample_df[sample_df["spread_t0"] <= median_spread]),
    ...
    stats_rows.extend(
        _build_bucket_stats(sample_df, "open_interest", "open_interest_t0")
    )
...
def main() -> None:
    ...
    tradable_df = sample_df[
        (sample_df["is_tradable"] == True) & (sample_df["signal_flag"] == 1)
    ].copy()
    ...
    tradable_run_count = (
        tradable_df["run_id"].nunique() if "run_id" in tradable_df.columns else 0
    )
    signal_ok = True
