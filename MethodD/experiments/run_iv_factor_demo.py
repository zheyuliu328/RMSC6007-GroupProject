...
def main():
    ...
    for col in iv_data.columns:
        factor_a[col] = IVFactorDefinition.compute_factor_version_a(iv_data[col], window=10)
        factor_b[col] = IVFactorDefinition.compute_factor_version_b(iv_data[col], window=10)
    
    print(f"✓ 计算了 Version A（简单标准化）和 Version B（MAD 标准化）")
    print(f"  Factor A 均值: {factor_a.mean().mean():.4f}, 标准差: {factor_a.std().mean():.4f}")
    print(f"  Factor B 均值: {factor_b.mean().mean():.4f}, 标准差: {factor_b.std().mean():.4f}")
    print()
    ...
    for col in factor_a.columns:
        signals_threshold[col] = SignalPolicy.threshold_strategy(factor_a[col], 
                                                                  long_threshold=-0.15,
    ...
    for col in factor_a.columns:
        signals_quantile[col] = SignalPolicy.quantile_strategy(factor_a[col], n_buckets=5)
    ...
    # 阈值策略回测
    result_threshold = backtest_runner.run_backtest(prices, signals_threshold, holding_period=5)
    
    # 分位数策略回测
    result_quantile = backtest_runner.run_backtest(prices, signals_quantile, holding_period=5)
    ...
    for holding_period in [3, 5, 7, 10]:
        result = backtest_runner.run_backtest(prices, signals_threshold, holding_period=holding_period)
        print(f"  持有期 {holding_period} 天: 总收益 {result['total_return']:.2%}, 夏普比率 {result['sharpe_ratio']:.4f}")
    print()
