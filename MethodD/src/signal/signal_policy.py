...
class SignalPolicy:
    ...
    @staticmethod
    def quantile_strategy(factor_series: pd.Series, n_buckets: int = 5) -> pd.Series:
        """
        分位数策略：
        - Top 分位数: 做空信号 (-1)
        - Bottom 分位数: 做多信号 (1)
...
def apply_triple_gate_signals(
    run_df: pd.DataFrame, config: TripleGateConfig
) -> pd.DataFrame:
    """
    在 run 级表上批量计算三重门控信号
...
class SignalProcessor:
    ...
    def __init__(self, holding_period: int = 7, filter_earnings: bool = True):
