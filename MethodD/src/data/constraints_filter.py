...
class EarningsWindowFilter:
    ...
    @staticmethod
    def filter_earnings_window(dates: pd.DatetimeIndex, 
                              ticker: str,
                              window_days: int = 5) -> pd.DatetimeIndex:
        """
...
class BorrowRateFilter:
    ...
    @staticmethod
    def filter_by_borrow_rate(tickers: List[str], 
                             max_rate: float = 0.02) -> List[str]:
        """
        按融券利率过滤（< 2%）
...
class ConstraintsAnalyzer:
    ...
    @staticmethod
    def analyze_constraints(ticker: str, 
                           start_date: str,
                           end_date: str) -> Dict:
        """
        分析约束条件对交易的影响
        ...
        return {
            ...
            'days_to_next_earnings': (earnings_info['next_earnings'] - datetime.now()).days if earnings_info['next_earnings'] else None
        }
