...
class RealIVDataLoader:
    ...
    def get_atm_iv_history(
        ticker: str, start_date: str, end_date: str, target_dte: int = 30
    ) -> pd.DataFrame:
        """
        获取 ATM 30D IV 历史数据
    ...
    while True:
        try:
            ...
            if not _should_retry(e):
                raise
            wait_seconds = (
                retry_schedule[attempt] if attempt < len(retry_schedule) else 60
            )
...
class CoveredCallPnLCalculator:
    ...
    def simulate_three_scenarios(
        ...
        call_premium_paid_up = (
            BlackScholesOption.call_price(
        ...
        results["scenario_2_flat"] = (
            CoveredCallPnLCalculator.calculate_covered_call_pnl(
                entry_price=entry_price,
