...
class IVFactorDefinition:
    ...
    def compute_both_versions(iv_series: pd.Series, window: int = 10) -> pd.DataFrame:
        ...
        return pd.DataFrame({"factor_a": factor_a, "factor_b": factor_b})
...
class FactorBucketizer:
    ...
    @staticmethod
    def bucketize_by_quantile(
        factor_series: pd.Series, n_buckets: int = 5
    ) -> pd.Series:
        """
        按分位数分组
    ...
    def get_top_bottom_signals(factor_series: pd.Series, n_buckets: int = 5) -> Dict:
        ...
        return {"top": top_indices, "bottom": bottom_indices}
...
class FactorNeutralizer:
    ...
    def neutralize_by_market_cap(
        factor_series: pd.Series, market_cap: pd.Series
    ) -> pd.Series:
        """
        按市值中性化因子
